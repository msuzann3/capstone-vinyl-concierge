...
- Added an up-front professor test-path instruction on the customer screen and documented the full connected-workflow test in the README. The protected all-customer aggregate still requires an authenticated `owner` role, while the same-session handoff remains available to the tester who created it.
...
- Removed numeric track-order badges, replaced the repeated Shelf Expansion album cards and unexplained tags with one prominent Suggested Exploration Areas panel, and added a dated sidebar new-release board with `Foreign Tongues` correctly labeled as coming July 10 rather than releasing this week.
- Removed Phoebe Bridgers and `Punisher` from the local recommendation catalog, Discogs seed list, customer recommendation inputs, synthetic owner-interest data, and live Firestore catalog; the client also filters that artist if a stale record reappears.
...
- Fixed the fixed-first-five fallback bias by removing the catalog-position score bonus and replacing it with a stable query-specific fallback order; the later feedback pass removed Phoebe Bridgers from the recommendation catalog entirely.
