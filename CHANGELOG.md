...
- Tightened recommendation ranking so live Firestore results are selected by top score instead of arbitrary Firestore document order, and added artist-affinity weighting so profiles like Jimmy Buffett / Kenny Chesney / Zac Brown favor coastal country, country rock, Americana, and folk-rock bridge records over generic classic rock.
- Added artist-affinity bridges for Ozzy Osbourne, Joni Mitchell, and the Beatles so mixed profiles can balance exact catalog matches with nearby records like Black Sabbath's `Paranoid` instead of collapsing into only one genre lane.
