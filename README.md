...
- Publish `firestore.rules` in the Firebase console before relying on customer session writes; the service account can seed Firestore data and update Auth authorized domains, but does not have permission to deploy rules through the Firebase CLI.
