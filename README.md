...
- Recommendation engine: Firestore-backed catalog reads in `src/recommender.ts`, with the local in-repo catalog retained as a fallback when the `albums` collection is empty or unavailable. The ranker weights selected genres, exact artist matches, and a small curated artist-affinity map for cases like coastal country / country rock. No Gemini or Google API key required.
...
- Firebase Auth state: Google sign-in is enabled, and the authorized domains include `localhost`, `vinyl-concierge.firebaseapp.com`, `vinyl-concierge.web.app`, and `msuzann3.github.io`.
